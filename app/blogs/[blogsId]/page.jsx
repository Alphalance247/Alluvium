...
// new/updated blog posts show up without a rebuild.
async function fetchArticle(blogsId) {
  const res = await fetch(`${environment?.baseUrl}api/blog/posts/${blogsId}/`, {
    cache: "no-store",
  });
...
export async function generateMetadata({ params }) {
  const blogsId = decodeURIComponent((await params).blogsId);
  const article = await fetchArticle(blogsId);
  return {
